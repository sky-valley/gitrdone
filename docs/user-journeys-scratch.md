...
| J2.3 | B is amended while C depends on it | Preserve the original lineage, derive reconciliation work, and keep any resulting held version current without pretending every later rebase is another B → B′ replacement | Executable repository proof |
...
| J3.2 | C conflicts mechanically with B′ | Preserve B, B′, and C; surface the conflict as durable judgement work; after C′ is accepted, replace captured C locally and replay only work created after it | Executable proof |
...
For J3.3, B′ remains in judgement: `grd status` explains the pending repository amendment, while accepted intent, canonical trunk, and the workspace remain unchanged and `grd sync` refuses reconciliation. J2.3 now permits B′ to be accepted without promoting stale C: C is derived as reconciliation work, clean replay creates C′ of the same Change, failure reuses the durable conflict path, and every result faces ordinary judgement. If C′ remains held while intent advances D → E, the repository records the honest C′@D → C″@E held-version rebase rather than repeating the old B → B′ claim. Multiple unseen amendments remain a later J3 deviation above.
