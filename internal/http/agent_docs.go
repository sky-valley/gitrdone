...
If an authenticated adapter cannot replay descendant C from submitted B onto accepted amendment B-prime at an exact current intent, first admit C through the ordinary proposal boundary, then record that existing descendant version as judgement work with POST /v1/repos/{repoID}/reconciliation-conflicts. New callers include expectedIntent; B-prime may be historical but must remain in that intent's accepted ancestry. Discover repository conflict history oldest-first with GET /v1/repos/{repoID}/reconciliation-conflicts using bounded limit and opaque cursor pagination, or inspect one conflict with GET /v1/repos/{repoID}/reconciliation-conflicts/{conflictID}. The POST requires Idempotency-Key, and the durable reportedBy field comes from authenticated authority rather than request JSON. Affected paths are optional, bounded diagnostics from the adapter; they are not the repository's content-conflict representation. Discovery is a read model over durable history, not a separate worker queue. Conflict reads derive awaiting_judgement, resolved, or superseded from immutable attempts, resolutions, and current intent. Internal judgement may record engine-produced C-prime as a new Version of C; if it remains held while intent advances, an internal held-version rebase creates another Version against current intent. Every resulting Version enters durable pending judgement until a separate promotion succeeds. Reads expose the ordered effective chain of held rebases and ordinary amendments so clients preserve repository-produced content. Neither conflict resolution nor held-version rebase is currently a public HTTP command.
...
gitrdone's native API exposes accepted intent, durable changes, immutable change versions, and durable reconciliation conflicts whose state is derived from immutable resolution history. Use canonical repo IDs, not namespace/name, as repository identity. Public discovery files are safe to scrape. Control, Git, and Git LFS operations require the appropriate tokens.
...
- [Inspect reconciliation conflict](%[1]s/v1/repos/{repoID}/reconciliation-conflicts/{conflictID}): GET durable judgement work, its derived state, optional immutable resolution, and bounded affected-path diagnostics.
