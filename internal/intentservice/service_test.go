...
func TestServiceAdmissionCreatesDurablePendingVersionWithoutPromotion(t *testing.T) {
	...
	pending, err := repository.PendingJudgements(context.Background(), intent.PendingJudgementQuery{Limit: 10})
	if err != nil {
		t.Fatalf("list pending judgements: %v", err)
	}
	if len(pending.Versions) != 1 || pending.Versions[0].ID != admission.Version.ID {
		t.Fatalf("pending judgements = %#v, want admitted version", pending.Versions)
	}
}

func TestServiceListsDurablePendingJudgements(t *testing.T) {
	initialContent := intent.ContentRef{Engine: "git", Revision: "aaaaaaaa"}
	...
	pending, err := service.PendingJudgements(context.Background(), "repo_123", intent.PendingJudgementQuery{Limit: 10})
	if err != nil {
		t.Fatalf("list pending judgements: %v", err)
	}
	if len(pending.Versions) != 1 || pending.Versions[0].ID != admission.Version.ID {
...
func TestServiceAmendmentRemainsPendingUntilExplicitPromotion(t *testing.T) {
	...
	pending, err := service.PendingJudgements(ctx, "repo_123", intent.PendingJudgementQuery{Limit: 10})
	if err != nil {
		t.Fatalf("list pending judgements: %v", err)
	}
	if len(pending.Versions) != 1 || pending.Versions[0].ID != amended.Version.ID {
		t.Fatalf("pending judgements = %#v, want only amended version", pending.Versions)
	}
