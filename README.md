...
Every operation that creates a judgeable Version now ends at a durable pending-judgement boundary: proposal, amendment, dependent reconciliation, conflict resolution, and held-version rebase. The repository's replayable ledger projection contains each latest unpromoted Version once; an idempotent retry does not duplicate it, a replacement Version supersedes it, and preparing promotion removes it because judgement has already produced a promotion attempt. The pending collection is currently an internal repository/service query rather than a public worker API. Claiming work, recording decisions, and executing judgement plans remain separate later slices.
