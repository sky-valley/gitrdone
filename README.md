...
| `GITRDONE_JUDGEMENT_WORKERS` | `0` | Number of asynchronous pending-work processors; `0` disables autonomous judgement |
| `GITRDONE_JUDGEMENT_MODEL` | `claude-sonnet-5` | Anthropic model ID supplied through the embedded Go pi provider |
| `ANTHROPIC_API_KEY` | required when judgement runs | Anthropic credential; never persisted in the repository ledger |
...
GET  /v1/repos/{repoID}/reviews
POST /v1/repos/{repoID}/review-responses
```
...
Every operation that creates a judgeable Version now ends at a durable pending-judgement boundary: proposal, amendment, dependent reconciliation, conflict resolution, and held-version rebase. The repository's replayable ledger projection contains each latest unpromoted Version once; an idempotent retry does not duplicate it, a replacement Version supersedes it, and preparing promotion removes it because judgement has already produced a promotion attempt.

When `GITRDONE_JUDGEMENT_WORKERS` is greater than zero, the server runs a storage-agnostic pending runner over one global stream. It uses bounded workers and expiring, generation-fenced in-memory leases; a crash loses only coordination state, so restart rediscovers the still-pending Version. Production wiring requires an explicit processor and currently uses four repository-governed concern arms evaluated by Anthropic through the embedded Go pi library. Processing is at least once, and promotion's durable CAS/idempotency—not the lease—fences the repository effect. Failed work releases its lease but receives a disposable per-runner exponential cooldown from five seconds to five minutes, preventing permanent guidance or model failures from becoming a one-second API-spend loop. Archived repositories are excluded and rechecked before any promotion attempt. Filesystem repository discovery is hidden behind the global pending-source interface, while a future Postgres implementation can query and lease pending Versions directly. Leases and retry cooldowns are disposable synchronization, not a second queue or mutable state on an immutable Version.

The evaluator reads `.gitrdone/purpose.md` and `.gitrdone/priorities.md` from the Version's accepted governing Intent, never from the candidate being judged. The current pilot priorities format uses one `## concern-name` section per independent arm, followed by a canonical `Reviewer:` email and a one-line natural-language `Prompt:`. Candidate content contributes only bounded Git change evidence. Missing or malformed accepted guidance fails closed and leaves the Version pending.

Concern assessments and human responses are immutable exact-Version ledger events. Each evaluated concern records a bounded opaque evaluator reference and prompt-contract revision so later inspection can distinguish model or interpretation changes without making a provider part of repository semantics. A matching concern derives a review obligation; only the authenticated subject assigned by that obligation may answer it. `approved` resolves the obligation, while `changes_requested` records the rationale and leaves it open for a later candidate or response. Promotion rechecks unresolved obligations at its durable boundary. Review cursors name immutable assessment order and remain valid when the cursor Version is promoted or superseded between pages. Review responses and their idempotency records survive filesystem-ledger restart; they are not lease or queue state.
...
The conflict collection GET returns durable conflicts oldest-first with bounded `limit` and opaque `cursor` pagination. This is repository history, not a separately persisted worker queue. An internal judgement operation may admit engine-produced C′ as a new Version of C and record an immutable resolution; C′ remains pending until a separate promotion operation succeeds. If an unresolved attempt or unpromoted resolution falls behind newer accepted intent, reads derive `superseded` while retaining the immutable facts. Internal engine work may rebase stale held C′ into C″ of the same Change, and C″ enters pending judgement in turn. Reads expose the ordered effective Version chain, including later held rebases and ordinary amendments, so the Git portal follows accepted repository-produced content instead of creating parallel work from obsolete C. There is intentionally no public conflict-resolution or held-version-rebase mutation endpoint yet.
...
grd sync
grd reviews
grd approve <review> -m "why this is acceptable"
grd request-changes <review> -m "what must change"
```

`grd submit` publishes the current committed content and admits it for judgement. The proposal request returns after durable admission, so the client reports judgement as pending and records a local continuation cursor. The separate runner may then promote it, hold it for assigned human review, or eventually perform richer judgement actions. Subsequent work can be submitted with an explicit dependency on that exact version. `grd reviews` lists only obligations assigned to the authenticated review-token subject and gives each a short local handle; response commands never require a person to type a Change or Version ID. Before responding, the client records both the opaque review mapping and a random operation key in local Git config. A lost response retries that exact operation, while deliberately repeating the same words after a confirmed intervening response creates a new immutable event.

`grd status` shows the last known relationship between the active workspace and its submitted parent, includes the rationale for a pending repository amendment, and only claims it is based on accepted intent after checking Git ancestry. If the repository amends and promotes that submitted version, `grd sync` fetches the accepted version, creates a recovery ref, and explains the repository rationale. A workspace still at the submitted version moves directly to the accepted amendment; clean local successor commits are replayed onto it. If replay conflicts, the Git adapter restores the original workspace, first admits C with ordinary Change/Version identity, then records durable reconciliation work against that existing version and waits for judgement. Git path diagnostics are optional evidence, not a custom conflict representation; jj-core remains the intended engine for first-class conflicted versions and automatic descendant rebasing.
