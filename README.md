...
| `GITRDONE_JUDGEMENT_WORKERS` | `0` | Number of asynchronous pending-work processors; `0` disables approve-all processing |
...
When `GITRDONE_JUDGEMENT_WORKERS` is greater than zero, the server runs a storage-agnostic pending runner over one global stream. It uses bounded workers and expiring, generation-fenced in-memory leases; a crash loses only coordination state, so restart rediscovers the still-pending Version. The current `ApproveAllProcessor` is temporary execution plumbing: it requests promotion against current intent, but it is not the eventual judgement or workflow model. Processing is at least once, and promotion's durable CAS/idempotency—not the lease—fences the repository effect. Archived repositories are excluded and rechecked before any promotion attempt. Filesystem repository discovery is hidden behind the global pending-source interface, while a future Postgres implementation can query and lease pending Versions directly. Leases are disposable synchronization, not a second queue or mutable state on an immutable Version.
